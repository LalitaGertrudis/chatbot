...
async function embedPDF(pdfBlob) {
    console.log("Embed PDF");
    ...
    vectorStore = await MemoryVectorStore.fromDocuments(splitDocs, embeddingsHF).catch(console.log);
    console.log("vectorStore done")
}

async function queryVectorStore(messages) {
    if (!vectorStore) {
...
async function main() {
    ...
    const blob = await readPDF("./art_22.pdf");
    console.log("Embedding pdf blob");
    await embedPDF(blob);
    console.log("Quering...");
    await queryVectorStore([{ role: "human", content: "quien es un don nadie? segun carlos murciano" }]);
}
